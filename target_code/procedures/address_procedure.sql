...
IS
    v_sql           VARCHAR2(1000);
    v_table_name    VARCHAR2(5);        -- Might need to change


BEGIN
    --Setting the nocount to check if it exists and declaring variables
    SET NOCOUNT ON;
    DECLARE @AddressID VARCHAR(38);

    --Get names 
    v_addr := 'address' || in_address_num;
    v_table_name := 'tbl_' || in_source_table_name;
